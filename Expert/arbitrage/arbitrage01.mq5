...
void OnTick()
  {
//---
   CheckOpenPosition("AUDUSD","USDCAD","AUDCAD",0.0015,1);
  }
//+------------------------------------------------------------------+
void CheckOpenPosition(string pair1,string pair2,string pair3,double ar_pip,double volume)
  {
   ...
   if(x_bid-price_pair3_ask>ar_pip)
     {
      //printf("BUY");
      ...
      //     
      while(!pair1Result)
         pair1Result=openPosition(pair1,volume,ORDER_TYPE_SELL,price_pair1_bid);
      while(!pair2Result)
         pair2Result=openPosition(pair2,NormalizeDouble(volume*price_pair1_bid,2),ORDER_TYPE_SELL,price_pair2_bid);
      while(!pair3Result)
         pair3Result=openPosition(pair3,NormalizeDouble((((100000*volume*price_pair1_bid)/price_pair2_bid)/price_pair3_ask)/100000,2),ORDER_TYPE_BUY,price_pair3_ask);

      if(pair1Result && pair2Result && pair3Result)
         closeAllPostitions();
     }
   else if(x_ask-price_pair3_bid<(-1*ar_pip))
     {
      //printf("SELL");
      ...
      while(!pair1Result)
         ...
         pair1Result=openPosition(pair1,NormalizeDouble(((100000/price_pair2_ask*volume)/price_pair1_ask)/100000,2),ORDER_TYPE_BUY,price_pair1_ask);
      while(!pair2Result)
         pair2Result=openPosition(pair2,volume,ORDER_TYPE_BUY,price_pair2_ask);
      while(!pair3Result)
         pair3Result=openPosition(pair3,NormalizeDouble((price_pair3_bid/(price_pair1_bid/price_pair2_bid*volume)/100000)*100000,2),ORDER_TYPE_SELL,price_pair3_bid);
...
//|                                                                  |
//+------------------------------------------------------------------+
bool openPosition(string pair,double volume,ENUM_ORDER_TYPE orderType,double price)
  {
